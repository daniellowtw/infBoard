...
function ServerSocketBroker(socket, roomId) {
    ...
    socket.on(SocketBroker.DELETE_SELECTED_CLIENT, function (message) {
        var nonce = message.nonce;
        var objIdToDelete = message.objId;
        dbClient.deleteObject(that.id, objIdToDelete, function (err) {
            if (err) {
                // TODO: add monitoring
                that.socket.emit("SYNACK", {nonce: nonce, err: err});
            } else {
                that.socket.emit("SYNACK", {nonce: nonce, res: "OK"});
                that.socket.broadcast.to(that.id).json.emit(SocketBroker.DELETE_SELECTED_SERVER, objIdToDelete)
            }
        })
    });
